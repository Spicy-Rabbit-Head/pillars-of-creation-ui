...
export function useBEM<B extends string, N extends string>(block: B, namespace: (isVar?: boolean) => N): NBEM<B, N>
export function useBEM<B extends string, N extends string>(block: B, namespace?: N | ((isVar?: boolean) => N)) {
  if (!namespace) {
    return <BEM<B>>{
      b: () => `${block}`,
      be: e => `${block}__${e}`,
